...
test('check that the location list renders propertly',async () => {
    const {getByText}= render(<ListOfLocations loadLocations={jest.fn()} ownLocations={testLocations} friendLocations={[]} setSelectedView={()=>{}}></ListOfLocations>)
...
test('check that with no location the loading squeletons appear',async () => {
    const {getByTestId}= render(<ListOfLocations loadLocations={jest.fn()} ownLocations={[]} friendLocations={[]} setSelectedView={()=>{}}></ListOfLocations>)
