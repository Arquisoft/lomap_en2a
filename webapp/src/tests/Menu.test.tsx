...
test('check menu contains 5 options', async () => {
  const { container } = render(<Menu loadLocations={jest.fn()} ownLocations={[]} friendLocations={[]}changeViewTo={jest.fn()}></Menu>)
  //we check there are 5 icons = svg
  expect(container.querySelectorAll('svg').length).toBe(5)
})

test('check menu expands when mouse enters', async () => {
  const { getByTestId } = render(<Menu loadLocations={jest.fn()} ownLocations={[]} friendLocations={[]} changeViewTo={jest.fn()} />
  );
